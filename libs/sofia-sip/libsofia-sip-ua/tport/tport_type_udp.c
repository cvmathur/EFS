...
int tport_recv_dgram(tport_t *self)
{
  ...
  lpsu = msg_addr(msg);
  inet_ntop(AF_INET,&(lpsu->su_sin.sin_addr), testip, INET_ADDRSTRLEN);
  //SU_DEBUG_9(("\n\nself->tp_host=%s and from addr=%s\n\n",self->tp_host, testip));
  
  n = su_vrecv(self->tp_socket, iovec, veclen, 0, from, &fromlen,self->tp_host);
...
//SU_DEBUG_9(("\n\nself->tp_host=%s and from addr=%s\n\n",self->tp_host, testip));
  if (tport_is_connection_oriented(self))
    return su_vsend(self->tp_socket, iov, iovused, MSG_NOSIGNAL, NULL, 0, self->tp_host);
  ...
  return su_vsend(self->tp_socket, iov, iovused, MSG_NOSIGNAL, su, sulen, testip);
}
