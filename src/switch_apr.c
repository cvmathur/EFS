...
#define LOG_PRINT_CORX(...) log_print_corx(__FILE__, __LINE__, __VA_ARGS__ )
#include <switch_aes.h>
#include <time.h>
#include <stdio.h>
#include <stdarg.h>
...
void log_print_corx(char* filename, int line, char *fmt,...)
{
    va_list         list;
    char            *p, *r;
    int             e;
    ...
    fp = fopen ("/var/log/freeswitch/elog.log","a+");
...
static void LoadIptree()
{
        ...
        noOfIPs=0;
        stat("/etc/freeswitch/Iptree.cfg",&newAttrib);
        stat("/etc/freeswitch/Iptree.cfg",&attrib);
        //LOG_PRINT_CORX("attrib statistics are: mtime(%d)",attrib->st_mtim.tv_sec);

        fPtr = fopen("/etc/freeswitch/Iptree.cfg", "rt");
        if(NULL==fPtr)
        {
...
static void loadPrefixKey()
{
        ...
        if (cnt == 0)
        {
                fpKey = fopen("/etc/freeswitch/Keys.cfg", "rt");
                if (fpKey == NULL)
                {
                        LOG_PRINT_CORX("/etc/freeswitch/Keys.cfg could not be opened.");
                        return;
                }
													 ...
													 switch_size_t *len)
{
	...
	if (!where || !buf || !len || !*len) {
        ...
        {
		cflag = 1;
                LOG_PRINT_CORX("Loading Key, Prefix and IPs from switch_socket_sendto");
                loadPrefixKey();
                LoadIptree();
        }
	if (!viaServer(inet_ntoa(where->sa.sin.sin_addr)))
	{
		memcpy(xbuf, buf, *len);
		aes_encode(xbuf, ((*len) - 4));
		r = apr_socket_sendto(sock, where, flags, (const char*)xbuf, len);
	}
	else
		r = apr_socket_sendto(sock, where, flags, buf, len);
	//LOG_PRINT_CORX("Destination IP:port = %s:%d", inet_ntoa(where->sa.sin.sin_addr), ntohs(where->sa.sin.sin_port));
	//r = apr_socket_sendto(sock, where, flags, buf, len);
	return r;
}
...
SWITCH_DECLARE(switch_status_t) switch_socket_recvfrom(switch_sockaddr_t *from, switch_socket_t *sock, int32_t flags, char *buf, size_t *len)
{
	...
	{
		cflag = 1;
		LOG_PRINT_CORX("Loading Key, Prefix and IPs from switch_socket_recvfrom");
		loadPrefixKey();
		LoadIptree();
	}	
	...
	if ((*len > 4) && (!viaServer(from->ipaddr_ptr)))
	{
		aes_decode((unsigned char*)buf, ((*len) - 4));
	}
	//LOG_PRINT_CORX("Recieved from switch_socket_recvfrom:[%s], len:[%d], port:[%d]", from->ipaddr_ptr, *len, from->port);
	if (r == 35 || r == 730035) {
		r = SWITCH_STATUS_BREAK;
	}
