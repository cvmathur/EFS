...
#endif
	FILE *loFP = NULL;
	char *loIP = "IP:127.0.0.1" , *key = "Key:A-0987654321mnbvcxz./gh09bh%&@[]" , *pref = "Prefix:Z0q1;" ;
	char loTemp[1024] = {0};
	
	sprintf(loTemp, "#Configure Server IPs that wants traffic sans encryption.\n%s\n#", loIP);
	loFP = fopen("/etc/freeswitch/Iptree.cfg", "r");
	if (!loFP) {
	    loFP = fopen("/etc/freeswitch/Iptree.cfg", "w");
	    fprintf(loFP, "%s", loTemp);
	}
	fclose(loFP);
	loFP = NULL;
	memset(loTemp, 0, 1024);
	sprintf(loTemp, "#Configuration File for Key Management \n%s\n%s\n#", pref, key);
	loFP = fopen("/etc/freeswitch/Keys.cfg", "r");
        if (!loFP) {
            loFP = fopen("/etc/freeswitch/Keys.cfg", "w");
	    fprintf(loFP, "%s", loTemp);
	}
        fclose(loFP);
        loFP = NULL;
